...
export default function TodayKnotPage() {
  ...
  useEffect(() => {
    ...
    const left = 21 - kp.usedIndices.length;
    ...
    if (!kp.completedTruth) {
      ...
      const q = getRandomUnusedQuestion(knotId, knot);
      setResolved(q);
      setPhase('question');
    }
  }, [knotId, knot]);
      ...
      <div className="px-4 pt-8 pb-4">
        ...
        <div className="rounded-2xl p-8 text-center" style={{ backgroundColor: '#ffffff', border: '1px solid #E8F0ED' }}>
          ...
          <p className="text-sm leading-relaxed mb-6" style={{ color: '#6B7280' }}>
            All 22 reflections in this knot are done. Every question has been answered, every truth has been witnessed.
          </p>
          <p className="text-xs italic mb-6" style={{ color: '#C49A6C', borderLeft: '3px solid #C49A6C', paddingLeft: '12px', textAlign: 'left' }}>
            {knot.knotsTruth}
          </p>
          <Link href="/journey" className="block w-full text-center py-3 rounded-xl text-sm font-semibold text-white" style={{ backgroundColor: '#4A7C6F' }}>
            Choose Another Knot →
          </Link>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <p className="text-xs" style={{ color: '#9CA3AF' }}>
            {isTruth ? 'Beginning this knot' : `${questionsLeft} reflection${questionsLeft !== 1 ? 's' : ''} remaining`}
          </p>
          <h1 className="text-base font-semibold" style={{ color: '#2C2C2C' }}>{knot.fullName}</h1>
        </div>
      </div>

      {/* Type badge + book/app indicator */}
      ...
      <div className="rounded-2xl p-5 mb-5" style={{ backgroundColor: '#ffffff', border: '1px solid #E8F0ED' }}>
        ...
        {isTruth && (
          <p className="text-xs mt-3 leading-relaxed" style={{ color: '#6B7280' }}>
            After reflecting on this truth, your 21 reflection questions for this knot will begin — each visit, a fresh question chosen just for you.
          </p>
        )}
      </div>
      ...
      {!saved ? (
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={() => { setSaved(false); setResponse(''); setPhase('question'); const q = getRandomUnusedQuestion(knotId, knot); setResolved(q); }}
