...
const features = [
  { icon: '🧵', title: '365 Days of Daily Reflection', desc: '17 knots × 21 questions each + 8 Movement Deep Dives = one full year of reflection.' },
...
/** Seat counter bar */
function FoundingSeatsBanner() {
  const pct = Math.round(((FOUNDING_SEAT_CAP - FOUNDING_SEATS_LEFT) / FOUNDING_SEAT_CAP) * 100);
  return (
    <div className="max-w-sm mx-auto rounded-2xl px-5 py-4 mb-8"
      style={{ backgroundColor: 'rgba(255,255,255,0.88)', border: '1.5px solid #C49A6C66' }}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold uppercase tracking-wide" style={{ color: '#7D5C2E' }}>
          Founding Member Seats
        </span>
        <span className="text-xs font-bold" style={{ color: '#C49A6C' }}>
          {FOUNDING_SEATS_LEFT} of {FOUNDING_SEAT_CAP} remaining
        </span>
      </div>
      {/* Progress bar — fills as seats are claimed */}
      <div className="h-2 rounded-full" style={{ backgroundColor: '#E8F0ED' }}>
      ...
      <p className="text-xs mt-2 text-center" style={{ color: '#9CA3AF' }}>
        Founding rate locked forever · ${FOUNDING_PRICE_MONTHLY}/mo after trial
      </p>
    </div>
  );
}
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <nav className="sticky top-0 z-50 border-b" style={{ backgroundColor: '#FAF7F2CC', backdropFilter: 'blur(12px)', borderColor: '#E8F0ED' }}>
          <div className="max-w-5xl mx-auto px-5 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/signup" className="text-sm font-semibold px-4 py-1.5 rounded-lg text-white" style={{ backgroundColor: '#4A7C6F' }}>Start Free Trial</Link>
            </div>
          </div>
        </nav>

        {/* Hero */}
        <section className="max-w-2xl mx-auto px-5 pt-16 pb-10 text-center">
          <div className="inline-block text-xs font-semibold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6"
            style={{ backgroundColor: '#C49A6C22', color: '#7D5C2E', border: '1px solid #C49A6C44' }}>
            🪢 Founding Member Seats Open — {FOUNDING_SEATS_LEFT} of {FOUNDING_SEAT_CAP} remaining
          </div>
          ...
          <p className="text-sm italic mb-8 px-4 py-2 rounded-xl inline-block"
            style={{ color: '#5C3D0E', backgroundColor: 'rgba(250,247,242,0.72)', backdropFilter: 'blur(4px)' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-4">
            <Link href="/signup"
              className="px-8 py-3.5 rounded-xl text-sm font-semibold text-white text-center hover:opacity-90 transition-opacity"
              style={{ backgroundColor: '#4A7C6F' }}>
              Start 14-Day Free Trial — ${FOUNDING_PRICE_MONTHLY}/mo
            </Link>
            <Link href="/dashboard"
              className="px-8 py-3.5 rounded-xl text-sm font-semibold text-center border hover:bg-[#E8F0ED] transition-colors"
              style={{ borderColor: '#4A7C6F', color: '#4A7C6F' }}>
              Explore Free — No Account Needed →
            </Link>
          </div>
          <p className="text-xs" style={{ color: '#5A5A5A' }}>
            14-day free trial · ${FOUNDING_PRICE_MONTHLY}/month after · Cancel anytime
          </p>
        </section>
        ...
        <section className="max-w-3xl mx-auto px-5 py-14">
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            ...
            {/* Free tier */}
            <div className="rounded-2xl p-6" style={{ backgroundColor: 'rgba(255,255,255,0.90)', border: '1px solid #E8F0ED' }}>
              <div className="inline-block text-xs font-semibold uppercase tracking-wide px-3 py-1 rounded-full mb-4"
                style={{ backgroundColor: '#E8F0ED', color: '#6B7280' }}>Always Free</div>
              <h3 className="text-lg font-bold mb-1" style={{ color: '#2C2C2C' }}>Free</h3>
              <div className="flex items-baseline gap-1 mb-4">
                <span className="text-3xl font-bold" style={{ color: '#6B7280' }}>$0</span>
              </div>
              <ul className="space-y-2 mb-6">
                {[
                  'All 17 knots',
                  '3 book questions per knot',
                  'No journal saving',
                  'No account required',
                ].map((item) => (
                  <li key={item} className="text-sm flex items-center gap-2" style={{ color: '#6B7280' }}>
                    <span>◦</span> {item}
                  </li>
                ))}
              </ul>
              <Link href="/dashboard"
                className="block w-full text-center py-3 rounded-xl text-sm font-semibold border hover:bg-[#E8F0ED] transition-colors"
                style={{ borderColor: '#4A7C6F', color: '#4A7C6F' }}>
                Explore Free →
              </Link>
            </div>

            {/* Builder / Founding tier */}
            <div className="rounded-2xl p-6 sm:col-span-1" style={{ backgroundColor: 'rgba(255,255,255,0.90)', border: '2px solid #4A7C6F', boxShadow: '0 4px 20px rgba(74,124,111,0.1)' }}>
              <div className="inline-block text-xs font-semibold uppercase tracking-wide px-3 py-1 rounded-full mb-4"
                style={{ backgroundColor: '#4A7C6F', color: '#ffffff' }}>
                {FOUNDING_SEATS_LEFT} Founding Seats Left
              </div>
              <h3 className="text-lg font-bold mb-1" style={{ color: '#2C2C2C' }}>Builder Tier</h3>
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-3xl font-bold" style={{ color: '#4A7C6F' }}>${FOUNDING_PRICE_MONTHLY}</span>
                <span className="text-sm" style={{ color: '#9CA3AF' }}>/ month</span>
              </div>
              <p className="text-xs mb-4" style={{ color: '#C49A6C', fontWeight: 600 }}>Founding rate · locked forever</p>
              <ul className="space-y-2 mb-6">
                {[
                  ...
                  <li key={item} className="text-sm flex items-center gap-2" style={{ color: '#2C2C2C' }}>
                    <span style={{ color: '#4A7C6F' }}>✓</span> {item}
                  </li>
                ))}
              </ul>
              <Link href="/signup"
                className="block w-full text-center py-3 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity"
                style={{ backgroundColor: '#4A7C6F' }}>
                Start Free Trial
              </Link>
            </div>
            ...
            <div className="rounded-2xl p-6" style={{ backgroundColor: 'rgba(250,247,242,0.85)', border: '1px solid #E8F0ED', opacity: 0.8 }}>
              <div className="inline-block text-xs font-semibold uppercase tracking-wide px-3 py-1 rounded-full mb-4"
                style={{ backgroundColor: '#E8F0ED', color: '#9CA3AF' }}>Coming Soon</div>
              <h3 className="text-lg font-bold mb-1" style={{ color: '#2C2C2C' }}>Full App</h3>
              <div className="flex items-baseline gap-1 mb-4">
              ...
              <div className="w-full text-center py-3 rounded-xl text-sm font-semibold" style={{ backgroundColor: '#E8F0ED', color: '#9CA3AF' }}>
                Join Builder to Be First Notified
              </div>
            </div>
        ...
        <section className="py-14 px-5 text-center" style={{ backgroundColor: '#4A7C6F' }}>
          <div className="max-w-xl mx-auto">
            ...
            <Link href="/signup"
              className="inline-block px-8 py-3.5 rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity"
              style={{ backgroundColor: '#C49A6C', color: '#ffffff' }}>
              Claim Your Founding Seat — {FOUNDING_SEATS_LEFT} Remaining
            </Link>
            <p className="text-xs mt-4" style={{ color: '#C8DED9' }}>
              Or{' '}
              <Link href="/dashboard" style={{ color: '#ffffff', textDecoration: 'underline' }}>
                explore free with no account
              </Link>
              {' '}— 3 book questions per knot, no signup required
            </p>
          </div>
        </section>
        ...
        <footer className="py-8 px-5 text-center border-t" style={{ borderColor: '#E8F0ED', backgroundColor: 'rgba(250,247,242,0.90)' }}>
          <p className="text-xs" style={{ color: '#9CA3AF' }}>
            ...
            <Link href="/signup" style={{ color: '#4A7C6F' }}>Start Free Trial</Link> ·{' '}
            <Link href="/dashboard" style={{ color: '#4A7C6F' }}>Explore Free</Link>
          </p>
